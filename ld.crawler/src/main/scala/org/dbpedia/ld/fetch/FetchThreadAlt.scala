...
class FetchThreadAlt(conf: FetchConfig, services: FetchServices) extends Runnable with Closeable {
  ...
  private def processHost(): Unit = {
    val altBatchJob = services.altWorkBalancer.getNextAltBatchJob

    var host: Option[String] = Some(altBatchJob.host)

    val hostFilterResult = hostFilter.tryHost(host.get)

    val iriDocs = altBatchJob.iris.flatMap({
      iriString =>
        try {
          val iri = IRI.apply(iriString, normalize = true)
          val doc = new IriDoc()
          doc.iri = (iri.toString)
          doc.hash = (iri.sha256sum)
          doc.location = (iri.locationIRI.toString)
          doc.locationHash = (iri.locationIRI.sha256sum)
          doc.host = (iri.host)
    ...
    if (hostFilterResult.accepted) {
      if (conf.skipExisting) {
        val (skipped, filteredDocs) = filterExistingDocs(iriDocs.asJava)
        log.info(s"SKIPPED $skipped/${iriDocs.size} @ $host")
        processIrisWithTimeOut(host.get, filteredDocs)
      } else {
        processIrisWithTimeOut(host.get, iriDocs.asJava)
      }
    } else if(conf.retryStatus != Integer.MIN_VALUE) {
      val (skipped, filteredDocs) = filterStatusDocs(iriDocs.asJava)
      log.info(s"SKIPPED (status != ${conf.retryStatus}) $skipped/${iriDocs.size} @ $host")
      processIrisWithTimeOut(host.get, filteredDocs)
    } else {
      skipIris(iriDocs.toIterator, hostFilterResult.error.get)
    }
